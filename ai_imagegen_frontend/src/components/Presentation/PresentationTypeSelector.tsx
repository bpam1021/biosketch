...
const PresentationTypeSelector: React.FC<PresentationTypeProps> = ({ onClose }) => {
  ...
  const loadTemplates = async () => {
    try {
      const response = await axiosClient.get('/api/v2/presentation-types/templates/');
  ...
  const handleAIGenerate = async () => {
    ...
    try {
      ...
      if (selectedType === 'document') {
        response = await axiosClient.post('/api/v2/presentation-types/generate_document_ai/', {
          prompt: aiPrompt,
        ...
        response = await axiosClient.post('/api/v2/presentation-types/generate_slides_ai/', {
          prompt: aiPrompt,
  ...
  const pollTaskStatus = async (taskId: string, type: 'document' | 'slide') => {
    ...
    const checkStatus = async () => {
      try {
        const response = await axiosClient.get(`/api/v2/presentation-types/check_generation_status/?task_id=${taskId}`);
