  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 p-4 shadow-sm">
      ...
      <div className="grid grid-cols-2 gap-4 text-sm">
        {job.num_samples > 0 && <div><span className="text-gray-500">Samples:</span><span className="ml-1 font-medium">{job.num_samples}</span></div>}
        {job.genes_quantified > 0 && <div><span className="text-gray-500">Genes:</span><span className="ml-1 font-medium">{job.genes_quantified.toLocaleString()}</span></div>}
        {job.significant_genes > 0 && <div><span className="text-gray-500">DEGs:</span><span className="ml-1 font-medium">{job.significant_genes.toLocaleString()}</span></div>}
        {job.cells_detected > 0 && <div><span className="text-gray-500">Cells:</span><span className="ml-1 font-medium">{job.cells_detected.toLocaleString()}</span></div>}
        {job.cell_clusters > 0 && <div><span className="text-gray-500">Clusters:</span><span className="ml-1 font-medium">{job.cell_clusters}</span></div>}
        {job.enriched_pathways > 0 && <div><span className="text-gray-500">Pathways:</span><span className="ml-1 font-medium">{job.enriched_pathways}</span></div>}
        {job.duration_minutes > 0 && <div><span className="text-gray-500">Duration:</span><span className="ml-1 font-medium">{formatDuration(job.duration_minutes)}</span></div>}
        {job.alignment_rate > 0 && <div><span className="text-gray-500">Alignment:</span><span className="ml-1 font-medium">{(job.alignment_rate * 100).toFixed(1)}%</span></div>}
      </div>
            ...
            <div className="mt-2 space-y-1">
              {job.pipeline_steps.map((step) => (
                <div key={step.step_number} className="flex items-center gap-2 text-xs">
                  ...
                  {step.duration_minutes > 0 && <span className="text-gray-400">({step.duration_minutes} min)</span>}
                </div>
              ))}
            </div>
          </details>
      ...
      {/* AI Interpretations */}
      {job.ai_interpretations?.length > 0 && (
        <div className="mt-3">
          <details className="group">
            <summary className="cursor-pointer text-sm text-gray-600 hover:text-gray-800">
            ...
            <div className="mt-2 space-y-2">
              {job.ai_interpretations.map((interpretation) => (
                <div key={interpretation.id} className="p-2 bg-purple-50 rounded text-xs">
                  <div className="font-medium text-purple-800">{interpretation.analysis_type.replace('_', ' ')}</div>
                  <div className="text-purple-700 mt-1 line-clamp-2">{interpretation.ai_response}</div>
                </div>
              ))}
            </div>
          </details>
