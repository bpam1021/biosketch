...
from users.models import PresentationTemplate, ChartTemplate

class Command(BaseCommand):
    help = 'Load initial presentation and chart templates'

    def handle(self, *args, **options):
        self.stdout.write('Loading initial templates...')
        ...
        for template_data in presentation_templates:
            ...
            if created:
                ...
                self.stdout.write(f'✓ Created presentation template: {template.name}')
            else:
                self.stdout.write(f'- Presentation template already exists: {template.name}')
        ...
        for template_data in chart_templates:
            ...
            if created:
                created_charts += 1
                self.stdout.write(f'✓ Created chart template: {template.name}')
            else:
                self.stdout.write(f'- Chart template already exists: {template.name}')
        
        self.stdout.write(
            self.style.SUCCESS(
                f'Successfully loaded {created_presentations} presentation templates '
                f'and {created_charts} chart templates!'
            )
        )