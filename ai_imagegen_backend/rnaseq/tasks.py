...
def process_downstream_analysis(dataset_id, analysis_config):
    """
    Process comprehensive downstream RNA-seq analysis with AI assistance
    """
    try:
        dataset = RNASeqDataset.objects.get(id=dataset_id)
        ...
        # Run all analysis types for comprehensive analysis
        
        if dataset.dataset_type == 'bulk':
            # Run all bulk RNA-seq analyses
...
def perform_bulk_clustering_pca(dataset, config):
    ...
    try:
        ...
        # Append to existing interpretation
        if dataset.ai_interpretation:
            dataset.ai_interpretation += f"\n\n## PCA & Clustering Analysis\n{ai_interpretation}"
        else:
            dataset.ai_interpretation = f"## PCA & Clustering Analysis\n{ai_interpretation}"
        dataset.save()
...
def perform_differential_expression(dataset, config):
    ...
    try:
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Differential Expression Analysis\n{ai_interpretation}"
        dataset.save()
...
def perform_pathway_enrichment(dataset, config):
    ...
    try:
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Pathway Enrichment Analysis\n{ai_interpretation}"
        dataset.save()
...
def perform_signature_correlation(dataset, config):
    ...
    try:
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Gene Signature Analysis\n{ai_interpretation}"
        dataset.save()
...
def perform_sc_clustering(dataset, config):
    ...
    try:
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Single-cell Clustering Analysis\n{ai_interpretation}"
        dataset.save()
        
    except Exception as e:
        logger.error(f"Single-cell clustering failed: {e}")

def perform_cell_type_annotation(dataset, config):
    """
    Perform cell type annotation for single-cell data
    """
    try:
        ...
        # Mock cell type annotation
        cell_types = ['T cells', 'B cells', 'NK cells', 'Monocytes', 'Dendritic cells', 'Neutrophils']
        
        for cluster in clusters:
            # Assign random cell type for demo
            cluster.cell_type = np.random.choice(cell_types)
            cluster.save()
        ...
        ai_interpretation = get_ai_interpretation(
            ...
            'comprehensive_analysis',
            {
                'annotated_clusters': clusters.count(),
                'cell_types_identified': list(set(c.cell_type for c in clusters)),
                'user_hypothesis': config.get('user_hypothesis', '')
            }
        )
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Cell Type Annotation\n{ai_interpretation}"
        dataset.save()
        
    except Exception as e:
        logger.error(f"Cell type annotation failed: {e}")

def perform_sc_differential_expression(dataset, config):
    """
    Perform single-cell differential expression analysis
    """
    try:
        # Similar to bulk but for single-cell clusters
        perform_differential_expression(dataset, config)
        
    except Exception as e:
        logger.error(f"Single-cell differential expression failed: {e}")

def perform_pseudotime_analysis(dataset, config):
    """
    Perform pseudotime analysis for single-cell data
    """
    try:
        if dataset.dataset_type != 'single_cell':
            return
        
        # Mock pseudotime analysis
        ai_interpretation = get_ai_interpretation(
            dataset,
            'comprehensive_analysis',
            {
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Pseudotime Analysis\n{ai_interpretation}"
        dataset.save()
        
    except Exception as e:
        logger.error(f"Pseudotime analysis failed: {e}")

def perform_cell_communication_analysis(dataset, config):
    """
    ...
    try:
        ...
        # Mock cell communication analysis
        ai_interpretation = get_ai_interpretation(
            dataset,
            'comprehensive_analysis',
            {
        ...
        if dataset.ai_interpretation:
            ...
            dataset.ai_interpretation = f"## Cell-Cell Communication Analysis\n{ai_interpretation}"
        dataset.save()
        
    except Exception as e:
        logger.error(f"Cell communication analysis failed: {e}")
...
def get_ai_interpretation(dataset, analysis_type, context_data):
    ...
    try:
        ...
        Please provide a detailed biological interpretation of these specific analysis results, including:
